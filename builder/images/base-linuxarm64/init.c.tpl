 ...
 *
 * PATCHED by node-av: Modified to return gracefully when library
 * cannot be loaded instead of crashing with assert/abort.
 */
...
// PATCHED: Graceful check that returns 0 instead of crashing
#define CHECK_GRACEFUL(cond, fmt, ...) do { \
    if(!(cond)) { \
      if (getenv("IMPLIB_DEBUG")) { \
        fprintf(stderr, "implib-gen: $load_name: " fmt " (graceful)\n", ##__VA_ARGS__); \
      } \
      return 0; \
    } \
  } while(0)
...
static int dlopened;
static int load_failed;  // PATCHED: Track if load already failed
...
static int load_library(void) {
  ...
  // PATCHED: Return early if we already know loading failed
  if (load_failed) {
    unlock();
    return 0;
  }
  ...
  // PATCHED: Graceful handling instead of CHECK/abort
  if (!lib_handle) {
    if (getenv("IMPLIB_DEBUG")) {
      fprintf(stderr, "implib-gen: $load_name: failed to load library via callback '$dlopen_callback' (graceful)\n");
    }
    load_failed = 1;
    unlock();
    return 0;
  ...
  if (!lib_handle) {
    if (getenv("IMPLIB_DEBUG")) {
      fprintf(stderr, "implib-gen: $load_name: failed to load library via dlopen: %s (graceful)\n", dlerror());
    }
    load_failed = 1;
    unlock();
    return 0;
...
#   endif
  ...
  h = lib_handle;
  // PATCHED: Graceful handling - return NULL if library failed to load
  if (!h) {
    if (getenv("IMPLIB_DEBUG")) {
      fprintf(stderr, "implib-gen: $load_name: failed to resolve symbol '%s', library not available (graceful)\n", sym_names[i]);
    }
    return 0;
  }
#endif
  ...
  CHECK_GRACEFUL(addr, "failed to resolve symbol '%s' via callback $dlsym_callback", sym_names[i]);
#else
  // Dlsym is thread-safe so don't need to protect it.
  addr = dlsym(h, sym_names[i]);
  CHECK_GRACEFUL(addr, "failed to resolve symbol '%s' via dlsym: %s", sym_names[i], dlerror());
#endif
...
void _${lib_suffix}_tramp_set_handle(void *handle) {
  ...
  lib_handle = handle;
  dlopened = 0;
  load_failed = 0;  // PATCHED: Reset failure state
}
